...
void invSubWord(uint32_t* temp) {
    ...
    *temp = sbox[tt[0]];
    *temp = *temp << 8;
    *temp = *temp | inv_sbox[tt[1]];
    *temp = *temp << 8;
    *temp = *temp | inv_sbox[tt[2]];
    *temp = *temp << 8;
    *temp = *temp | inv_sbox[tt[3]];
}
...
void encrypt8(uint8_t* rk8, uint8_t* a){
    ...
    for(int j = 1; j<10; j++){
        ...
        shiftRows(a);
        mixColumns(a);
        otfKeyExpansion(rk, j);
        addRoundKey(rk, a);
    }
        subBytes(a);  
        shiftRows(a);
        otfKeyExpansion(rk, 10);
        addRoundKey(rk, a);
}
